...
当前代码版本 `0.2.2`：修正原生自动化的工作区标识与微信投递目标绑定，并按“自动化关联 + Agent 标签”合计保留最新五条通知历史。摘要任务完成不代表微信已送达，必须另行验证发送链路。路线图中的 `/刷新` 和微信远程计划审批仍未实现，具体限制见下文。第一版发布标签不被覆盖。
...
每个增量监控轮次在通知工作区内，将符合以下两项的历史视为通知任务：存在非空 `cron_automation_id` 自动化关联；会话标题或所属自动化标题含 `[zcode]`、`[codex]`、`[claudecode]`、`[dsh]` 中的 Agent 标签。所有来源合计保留最新五条，不是每种来源五条。按任务更新时间、创建时间排序；显式 `baseline` 和首次静默基线不清理，下一次增量轮次开始整理。

不再要求旧 outbox/账本、通知器专用 ID 前缀或父自动化仍存在，因此旧版带标签通知也可整理。无自动化关联的普通会话、没有 Agent 标签的自动化、运行中或当前可识别的等待审批/输入会话不删除。置顶、归档或手动改名并不取消“自动化 + 标签”的归属；改名后仍有标签的已结束记录也纳入五条规则。其他工作区/身份不在本次清理范围，界面全部会话总数不要求等于五条。
